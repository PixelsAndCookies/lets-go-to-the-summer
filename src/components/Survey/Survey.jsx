...
export const Survey = (props) => {
    ...
    const [cookie, setCookie] = useState(() => {
        if (
            ...
            typeof document.cookie.split(';').find((row) => row.startsWith(Name)) != 'undefined'
        )
            return true
    })
            ...
            if (selectedOption !== null) {
                try {
                    const response = await fetch(
                        apiEndpoint + '?name=' + Name + '&id=' + selectedOption
                    )
                    const data = await response.json()
                    setSurveyData(data)
                    setCookie(() => {
                        ...
                        <div key={option.id}>
                            <div className="input">
                                ...
                                <label htmlFor={option.id}>
                                    {option.text}
                                    <span>
                                        {Math.round((option.count * 100) / surveyData.total)}%
                                    </span>
                                </label>
                            </div>
